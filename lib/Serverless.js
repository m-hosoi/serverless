...
class Serverless {
  ...
  initCredentials(cb) {
    if(!process.env.SERVERLESS_ADMIN_AWS_PROFILE){
      cb();
      return;
    }
    let profiles = awsMisc.profilesMap();
    if(profiles[process.env.SERVERLESS_ADMIN_AWS_PROFILE]){
      cb();
      return;
    }
    awsMisc.initRole(profiles, process.env.SERVERLESS_ADMIN_AWS_PROFILE, (credentials) => {
      this.config.awsAdminKeyId     = credentials.AccessKeyId;
      this.config.awsAdminSecretKey = credentials.SecretAccessKey;
      this.config.sessionToken = credentials.SessionToken;
      cb();
    });
  }
