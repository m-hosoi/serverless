...
class ServerlessState {
  ...
  getComponents(options) {
    ...
    if (!options.paths.length) {
      return options.returnPaths ? allComponents.map(function(d) { return d._config.sPath }) : allComponents;
    }
  ...
  getFunctions(options) {
    ...
    // Return if no paths
    if (!options.paths.length) {
      return options.returnPaths ? allFunctions.map(function(d) { return d._config.sPath }) : allFunctions;
    }
    ...
    for (let i = 0; i < allFunctions.length; i++) {
      ...
      for (let j = 0; j < options.paths.length; j++) {
        ...
        // if (func._config.sPath.endsWith(options.paths[j])) {
        if (func._config.sPath.split('/').pop() === options.paths[j].split('/').pop()) {
