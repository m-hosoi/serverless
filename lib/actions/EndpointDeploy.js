...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class EndpointDeploy extends SPlugin {
    ...
    _validateAndPrepare() {
      ...
      if (_this.S.cli &&
        ...
        if (!_this.evt.options.paths.length) {
          throw new SError(`No endpoints found in this location: ` + sPath);
        }

        SCli.log('Deploying all endpoints in: ' + sPath + '...');
      }

      // If --all is selected, load all paths
      if (_this.evt.options.all) {
        _this.evt.options.paths = _this.S.state.getEndpoints({ returnPaths: true });
      }
    ...
    _deployEndpointsByRegion(region) {
      ...
      if(_this.S.config.sessionToken){
        awsConfig.sessionToken = _this.S.config.sessionToken;
      }
          ...
          return new BbPromise(function(resolve, reject) {
            ...
            async.eachSeries(_this.evt.options.paths, function(path, eCb) {
              ...
              return BbPromise.try(function() {
                  ...
                  endpoint = _this.S.state.getEndpoints({ paths: [path] })[0];

                  if (!endpoint) throw new SError(`Endpoint could not be found: ${path}`);
                ...
                .catch(function(e) {
                  ...
                  _this.failed[region].push({
                    sPath:            endpoint ? endpoint._config.sPath : path,
