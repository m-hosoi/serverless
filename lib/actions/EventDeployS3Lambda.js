  ...
  class EventDeployS3Lambda extends SPlugin {
    ...
    eventDeployS3Lambda(evt) {
      ...
      return _this.Lambda.removePermissionPromised(params)
        ...
        .then(function(data) {
          return _this.S3.getBucketNotificationConfigurationPromised({
            Bucket: populatedEvent.config.bucket,
          });
        })
        .then(function(data) {
          data.LambdaFunctionConfigurations = data.LambdaFunctionConfigurations.filter(v => v.Id !== populatedEvent.config.id);
          data.LambdaFunctionConfigurations.push({
            Id: populatedEvent.config.id,
            Events: populatedEvent.config.bucketEvents,
            LambdaFunctionArn: lambdaArn,
            Filter: {
              Key: {
                FilterRules: populatedEvent.config.filterRules
              },
            }
          });
          let params = {
