...
module.exports.getEnvFileAsMap = function(Serverless, region, stage) {
  ...
  if (stage == 'local') {
    ...
    if(Serverless.config.sessionToken){
      awsConfig.sessionToken = Serverless.config.sessionToken;
    }
...
module.exports.initRole = function(profiles, roleName, cb){
  let configPath = path.join(this.getConfigDir(), 'config');
  let config = AWS.util.ini.parse(AWS.util.readFileSync(configPath));
  config = config['profile ' + roleName];
  if(!config){
    console.log('profile ' + roleName + ' not found');
    return;
  }

  let sourceProfile = profiles[config.source_profile];
  if(!sourceProfile){
    console.log('source profile ' + config.source_profile + ' not found');
    return;
  }
